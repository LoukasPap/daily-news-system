...
for i in range(1000):
    provider = fake.random_element(elements=('gmail', 'hotmail', 'yahoo', 'outlook', 'aueb', 'uoa'))
    fn = fake.first_name().lower()
    user: User = User(
        username = fn + fake.bothify("_##??###??#"),
        email = fn + "." + fake.bothify(text=f"##???_{provider}.com").replace("_", "@"),
        password = fake.bothify(text="??##???#??#")
    )

    # insert the new user
    response = users.insert_one(dict(user))
     
    # for each user take a sample
    ...
    for sample in samples_100:
        ...
        if will_like:
            ...
            response = articles.update_one(
                {"_id": sample["_id"]},
                {"$addToSet": {"liked_by": user.username},})
    ...
    print(f"Finished adding user #{i}")
    # break
